...
public class TimerWheel implements Timer {
     ...
     * @throws IllegalArgumentException if {@code tickDuration} is <= 0
     */
    public TimerWheel(ThreadFactory threadFactory, long tickDuration, TimeUnit unit) {
    ...
    public TimerWheel(ThreadFactory threadFactory,
        ...
        // 给定一个deadline的定时任务，其应该呆的格子=deadline%wheel.length.但是%操作是个相对耗时的操作，所以使用一种变通的位运算代替；
        // 因为一圈的长度为2的n次方，mask = 2^n-1后低位将全部是1，然后 deadline & mask == deadline % wheel.length
    ...
    private final class Worker implements Runnable {
        ...
        public void run() {
            ...
            do {
                // 返回的是当前的 nanoTime - startTime
                // 也就是返回的是每 tick 一次的时间间隔
        ...
        private void transferTimeoutsToBuckets() {
            ...
            for (int i = 0; i < 100000; i++) {
                ...
                /* 如果任务在 timeouts 队列里面放久了, 以至于已经过了执行时间,
                   这个时候就使用当前 tick, 也就是放到当前 bucket, 此方法调用完后就会被执行 */
         ...
         * 可以想象一下在时钟的秒钟上面秒与秒之间的时间是需要等待的，
         * 那么 waitForNextTick 这个方法就是根据当前的时间计算出跳动到下个时间的间隔时间，
         * 并进行sleep操作，然后返回当前时间距离时间轮启动时间的时间段。
         * <p>
         *
        ...
        private long waitForNextTick() {
            ...
            for (; ; ) {
                ...
                // 计算需要 sleep 的时间, 之所以加999999后再除10000000,前面是1所以这里需要减去1，才能计算准确
                /* 还有通过这里可以看到，其实线程是以睡眠一定的时候再来执行下一个 tick 的任务的，
                   这样如果 tick 的间隔设置的太小的话，系统会频繁的睡眠然后启动，*/
                // 其实感觉影响部分的性能，所以为了更好的利用系统资源步长可以稍微设置大点。
                long sleepTimeMs = (deadline - currentTime + 999999) / 1000000;

                if (sleepTimeMs <= 0) {
                    // sleepTimeMs 小于 0 表示走到了下一个时间轮位置
